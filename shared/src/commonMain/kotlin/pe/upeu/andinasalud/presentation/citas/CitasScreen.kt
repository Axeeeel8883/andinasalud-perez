...
@Composable fun CitasScreen(citas: List<Cita>, busqueda: String, filtro: String, buscar: (String) -> Unit,
    filtrar: (String) -> Unit, detalle: (Long) -> Unit, solicitar: () -> Unit) {
    Column(Modifier.fillMaxSize().padding(horizontal = 20.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        ...
        Button(onClick = solicitar, modifier = Modifier.fillMaxWidth()) { Text("Solicitar cita") }
